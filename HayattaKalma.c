...
int main() {
    // Rastgele sayý üretimini baþlat (Her çalýþtýrýþta farklý sonuçlar için)
    srand(time(NULL));
    ...
    do {
        ...
        printf("\nNe yapmak istersin? > ");
        scanf(" %c", &komut); 
        komut = toupper(komut); // Küçük harf girilse bile büyütür
        ...
        switch (komut) {
            ...
            case 'A': // Avlan
                ...
                if (enerji >= 15) {
                    ...
                    // Mantýksal Operatörler (&&, ||) kullanýmý
                    // %40 ihtimalle yemek bulur VE þanslýysa 2 yemek bulur
                    if (sans > 60) {
                        yemek++;
            ...
            case 'P': // Þifreli Ýlerleme (DO-WHILE) 
                ...
                int dogruSifre = 17; // Örnek sabit þifre
